...
// - Mark Tongol Homework 2

function cboDropDown(){
//paraClear()
    glossary.forEach(cboGlossaryClass => {
        let cboValue = document.getElementById('cboClassValues').selectedIndex;
        
        if( cboValue === cboGlossaryClass.class){
            newCboElement()
            //console.log(`${cboGlossaryClass.class} - ${cboGlossaryClass.term}`);
        }
    
    function newCboElement(){
        const newCboElementPara = document.createElement('p');
        let node = document.createTextNode(`${cboGlossaryClass.class} - ${cboGlossaryClass.term}`);
        newCboElementPara.appendChild(node);
        const element = document.getElementById('div1');
        element.appendChild(newCboElementPara);
     }
    })
}
...
function btnSearchClick(){
    ...
    let txtValue = document.querySelector('input').value;  //called in line14 eButton
    let upperTxtValue = txtValue.toUpperCase();

    glossary.forEach(function(glossaryOut){
      
        let upperGlossaryOut = glossaryOut.term.toUpperCase()
        let findTxtContains = upperGlossaryOut.indexOf(upperTxtValue);  // finding values that contains

             function newElement(){
                const newElementPara = document.createElement('p');
                let node = document.createTextNode(`${glossaryOut.term} - ${glossaryOut.definition}`);
                newElementPara.appendChild(node);
                const element = document.getElementById('div1');
                element.appendChild(newElementPara);
             }

            if(findTxtContains !== -1){
                newElement()               
                //console.log(`${glossaryOut.term} - ${glossaryOut.definition}`);
